...
L.Storage.Map.include({
    ...
    edit: function () {
        ...
        credits.appendChild(creditsBuilder.build());xiongjiabin 实在没有发现什么有价值的用处
    */
        ...
        var syncVideo = L.DomUtil.createFieldset(container, L._('Sync Video'));
        var slideshowFields = [
            ['options.syncVideoEnable', {handler: 'Switch', label: L._('Enable Sync Video')}],
            ['options.syncVideoUrl', {handler: 'BlurInput', label: L._('Sync Video Control')}],
        ];

        var slideshowBuilder = new L.S.FormBuilder(this, slideshowFields);
        syncVideo.appendChild(slideshowBuilder.build());
