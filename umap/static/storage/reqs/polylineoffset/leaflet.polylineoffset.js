L.PolylineOffset = {
  translatePoint: function(pt, dist, radians) {
    return L.point(pt.x + dist * Math.cos(radians), pt.y + dist * Math.sin(
      radians));
  },
  ...
  */
  joinSegments: function(s1, s2, offset, joinStyle) {
    ...
    switch (joinStyle) {
      ...
      case 'cut':
        jointPoints = [
          this.intersection(s1.offset[0], s1.offset[1], s2.original[0],
            s2.original[1]),
          this.intersection(s1.original[0], s1.original[1], s2.offset[0],
            s2.offset[1])
        ];
        break;
      case 'straight':
      ...
      case 'intersection':
      default:
        jointPoints = [this.intersection(s1.offset[0], s1.offset[1], s2.offset[
          0], s2.offset[1])];
    }
  ...
  */
  circularArc: function(s1, s2, distance) {
    ...
    if (endAngle > startAngle + Math.PI) {
      return [this.intersection(s1.offset[0], s1.offset[1], s2.offset[0],
        s2.offset[1])];
    }
...
L.Polyline.include({
  ...
  setOffset: function(offset) {
    ...
    var results = []
    this._projectOffsetsLatlngs(this._latlngs, offset,results);
    this._latlngs = results
    this.redraw();
    return this;
  },

  _projectOffsetsLatlngs: function(latlngs, offset, result) {
    ...
    if (flat) {
      ring = [];
      for (i = 0; i < len; i++) {
        ring[i] = this._map.project(latlngs[i],18);
      }
      ...
      for(i = 0, len = ring.length; i < len; i++){
        newLatLngs[i] = this._map.unproject(ring[i],18)
      }

      result.push(newLatLngs);
    } else {
      for (i = 0; i < len; i++) {
        this._projectOffsetsLatlngs(latlngs[i], offset, result);
      }
    }
  }
